...
meta_data$Neutrophil_count<-scale(meta_data$Neutrophil_count, center=T, scale=T)
meta_data$Lymphocyte_count<-scale(meta_data$Lymphocyte_count, center=T, scale=T)
meta_data$Monocyte_count<-scale(meta_data$Monocyte_count, center=T, scale=T)
meta_data$Eosinophil_count<-scale(meta_data$Eosinophil_count, center=T, scale=T)
...
DEseqDS<-DESeqDataSetFromMatrix(countData=filtered_raw_counts, colData=meta_data, design = ~ Age + Diabetes + Neutrophil_count + Lymphocyte_count + Monocyte_count + Eosinophil_count + Ethnicity)
...
write.csv(results, file="cell_count_corrected_de_results.csv")
